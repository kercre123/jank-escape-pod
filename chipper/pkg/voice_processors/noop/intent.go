...
func (s *Server) ProcessIntent(req *vtt.IntentRequest) (*vtt.IntentResponse, error) {
	...
	for {
		...
		fileBytes1, _ := ioutil.ReadFile("/tmp/utterance1")
		...
		if _, err := os.Stat("./armarch"); err == nil {
			...
			fileBytes2, _ := ioutil.ReadFile("/tmp/utterance2")
			...
			fileBytes3, _ := ioutil.ReadFile("/tmp/utterance3")
			...
			fileBytes4, _ := ioutil.ReadFile("/tmp/utterance4")
